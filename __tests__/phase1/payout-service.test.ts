...
describe("Phase 1 manual payout workflow", () => {
  ...
  it("prevents duplicate active payout requests for the same chef and amount", async () => {
    mockedPayoutRepository.findChefProfile.mockResolvedValue({ id: "chef-1", isApproved: true })
    mockedPayoutRepository.getCompletedBookingsWithPayments.mockResolvedValue([
      { payments: { totalAmount: 200, commissionAmount: 40 } },
    ])
    mockedPayoutRepository.listPayouts.mockResolvedValue([{ id: "payout-existing" }])

    await expect(payoutService.createPayout("user-1", 100)).rejects.toThrow("DUPLICATE_ACTIVE_PAYOUT")
  })
