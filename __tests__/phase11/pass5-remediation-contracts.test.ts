...
describe("Pass 5 remediation contracts", () => {
  ...
  it("persists client profile text and social settings through an authenticated API", () => {
    ...
    expect(route).toContain("username: payload.username")
    expect(route).toContain("bio: payload.bio")
    expect(route).toContain("website: payload.website")
    expect(route).toContain("socialProfile: payload.socialProfile")
    expect(route).toContain("id: { not: session.user.id }")
    expect(settingsPage).toContain("username: true")
    expect(settingsPage).toContain("socialProfile: true")
