...
const payoutCurrencyCodes = ['GBP', 'USD', 'EUR', 'KES'] as const;

const createPayoutSchema = z.object({
  amount: z.number().positive(),
  currency: z.enum(payoutCurrencyCodes).optional(),
});
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { amount, currency } = createPayoutSchema.parse(body);
    const payout = await payoutService.createPayout(userId, amount, currency);
    ...
    if (error instanceof Error && error.message.startsWith('INSUFFICIENT_BALANCE:')) {
      const [, currency = 'GBP', available = '0'] = error.message.split(':');
      return NextResponse.json({
        error: `Insufficient balance. Available: ${formatCurrency(Number(available), currency)}`
      }, { status: 400 });
    }
