"use client"

import { FileText, ChefHat, Calendar, DollarSign, ArrowRight, Search, Edit, Clock, TrendingUp } from "lucide-react"
import Link from "next/link"

interface EngagingQuickActionsProps {
  availableRequests: number
  activeBookings: number
}

export function EngagingQuickActions({ availableRequests, activeBookings }: EngagingQuickActionsProps) {
  const actions = [
    {
      title: 'Find new bookings',
      subtitle: 'Browse available requests',
      icon: <Search className="h-5 w-5" />,
      href: '/dashboard/chef/requests',
      badge: availableRequests > 0 ? availableRequests : null,
      color: 'blue' as const
    },
    {
      title: 'Update your profile',
      subtitle: 'Attract more clients',
      icon: <Edit className="h-5 w-5" />,
      href: '/dashboard/chef/profile',
      badge: null,
      color: 'purple' as const
    },
    {
      title: 'Set your schedule',
      subtitle: 'Manage availability',
      icon: <Clock className="h-5 w-5" />,
      href: '/dashboard/chef/availability',
      badge: null,
      color: 'green' as const
    },
    {
      title: 'View your bookings',
      subtitle: 'Track active requests',
      icon: <Calendar className="h-5 w-5" />,
      href: '/dashboard/chef/bookings',
      badge: activeBookings > 0 ? activeBookings : null,
      color: 'orange' as const
    },
    {
      title: 'Track earnings',
      subtitle: 'Monitor your income',
      icon: <TrendingUp className="h-5 w-5" />,
      href: '/dashboard/chef/payouts',
      badge: null,
      color: 'green' as const
    }
  ]

  const getColorClasses = (color: string) => {
    switch (color) {
      case 'blue': return 'bg-blue-50 text-blue-600 border-blue-200'
      case 'purple': return 'bg-purple-50 text-purple-600 border-purple-200'
      case 'green': return 'bg-green-50 text-green-600 border-green-200'
      case 'orange': return 'bg-orange-50 text-orange-600 border-orange-200'
      default: return 'bg-gray-50 text-gray-600 border-gray-200'
    }
  }

  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <h3 className="text-lg font-semibold text-gray-900">Quick Actions</h3>
        <p className="text-sm text-gray-600">Get started with these features</p>
      </div>
      
      <div className="space-y-3">
        {actions.map((action) => (
          <Link key={action.title} href={action.href}>
            <div className="group flex items-center justify-between p-4 bg-white border border-gray-200 rounded-xl hover:bg-gray-50 hover:shadow-sm transition-all duration-300 hover:translate-x-2">
              <div className="flex items-center gap-4 flex-1">
                <div className={`w-12 h-12 rounded-xl ${getColorClasses(action.color)} flex items-center justify-center`}>
                  {action.icon}
                </div>
                <div className="flex-1">
                  <p className="font-semibold text-gray-900 text-lg">{action.title}</p>
                  <p className="text-sm text-gray-600">{action.subtitle}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                {action.badge && (
                  <span className="bg-gray-100 text-gray-700 text-sm px-3 py-1 rounded-full font-medium">
                    {action.badge}
                  </span>
                )}
                <ArrowRight className="h-5 w-5 text-gray-400 group-hover:text-gray-600 transition-colors" />
              </div>
            </div>
          </Link>
        ))}
      </div>
    </div>
  )
}
